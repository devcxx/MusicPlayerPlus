...
HttpClient2 *HttpClient2::instance = NULL;

double HttpClient2::downloadFileLength = -1;
curl_off_t HttpClient2::resumeByte = -1;
time_t HttpClient2::lastTime = 0;

bool HttpClient2::stopCurl = false;
...
HttpClient2 *HttpClient2::getInstance()
{
    if (instance == NULL)
	{
...
void HttpClient2::destroyInstance()
{
    if (instance != NULL)
	{
        stopCurl = true;
...
void HttpClient2::zlog(const char *date, const char *time, const char *file, const int line, const char *func, const char *str)
{
#ifdef Z_DEBUG
	FILE *fp = NULL;
#ifdef _WIN32
...
int HttpClient2::HttpGet(const string requestURL, const string saveTo, void *sender, progress_info_callback cb)
{
    ...
    do
    {
        ...
        ret |= curl_easy_setopt(easy_handle, CURLOPT_WRITEFUNCTION, &HttpClient2::write_callback);
        ...
        ret |= curl_easy_setopt(easy_handle, CURLOPT_XFERINFOFUNCTION, &HttpClient2::progress_callback);
...
size_t HttpClient2::write_callback(char *buffer, size_t size, size_t nmemb, void *userdata)
{
...
int HttpClient2::progress_callback(void *userdata, curl_off_t dltotal, curl_off_t dlnow, curl_off_t ultotal, curl_off_t ulnow)
{
    (void)ultotal;
    (void)ulnow;
...
p_off_t HttpClient2::getLocalFileLength(string path)
{
	p_off_t ret;
...
// Get the file size on the server
double HttpClient2::getDownloadFileLength(string url)
{
    CURL *easy_handle = NULL;
	int ret = CURLE_OK;
	double size = -1;
